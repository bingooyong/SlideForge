...
## 如何使 Skill 与命令可用（独立仓库维护）

1. **命令与 Skill 均在本仓库内**
   - 命令定义位于 **`project-docs-governance/commands/`**（7 个 `doc-*.md`），便于作为独立 GitHub 仓库维护。
   - Skill 内容：`SKILL.md`、`references/`、`templates/`、`agents/` 均在本目录下。

2. **在使用方项目中的安装**
   - 详见 **[INSTALL.md](INSTALL.md)**。简要方式：
     - 将本仓库置于使用方项目根目录下 `project-docs-governance/`（submodule 或复制）。
     - 将 `project-docs-governance/commands/*.md` 复制到使用方项目的 `.claude/commands/`，或建立符号链接。
   - 命令内会优先读取 `project-docs-governance/SKILL.md` 与 `project-docs-governance/references/`（相对于使用方项目根）。
...
3. **Slash 命令列表**
   ...
   - 执行前会自动读取 Memory ROOT（`.apm/Memory/Memory_Root.md`）与本 Skill。
